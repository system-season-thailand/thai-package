...
// Dedicated Supabase client for close-sell (different project from main app)
const closeSellSupabaseUrl = 'https://lpyfcvjljejmxgoynbxb.supabase.co';
const closeSellSupabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxweWZjdmpsamVqbXhnb3luYnhiIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDY4ODk4OTYsImV4cCI6MjA2MjQ2NTg5Nn0.Ml7ICH8BoBZFbdRW-hOaN3OFX5j386QR_z0C4KmTt5k';
const closeSellSupabase = (typeof window !== 'undefined' && window.__supabaseCreateClient)
  ? window.__supabaseCreateClient(closeSellSupabaseUrl, closeSellSupabaseKey)
  : null;
...
async function ensureCloseSellCache() {
  ...
  if (!closeSellSupabase) {
    console.error("Close-sell Supabase client not available (ensure Supabase script loads before this file).");
    return;
  }
  try {
    const { data, error } = await closeSellSupabase.rpc("get_all_public_tables_data");
    if (error) {
