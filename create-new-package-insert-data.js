...
createHotelsDataFunction = function () {
    ...
    if (document.getElementById('hotel_location_input_id').style.display !== 'none') {
        ...
        const _hotelDbData_create = _findHotelRoomData(hotelNameReadyText);
        let hotelLocationReadyText = _hotelDbData_create ? _hotelDbData_create.hotelLocation : '';
...
let hotelRowImageFallbackClickBound = false;
let hotelRowImageGlobalDebugBound = false;
let lastHotelRowImageTriggerAt = 0;
let lastHotelRowImageTriggerRowId = '';

function logHotelRowImageDebug(step, payload = {}) {
...
function triggerHotelRowImageController(target, source) {
    ...
    if (
        ...
        logHotelRowImageDebug('popup trigger skipped (dedup)', {
            source,
            rowId: row.id,
            elapsedMs: now - lastHotelRowImageTriggerAt
        });
        return;
    }
    ...
    logHotelRowImageDebug('popup trigger fired', {
        source,
        rowId: row.id,
        imageSrc: clickedImage.getAttribute('src')
    });

    hotelRowImageControllerFunction({ target: clickedImage });
}

function ensureHotelRowImageGlobalDebugListeners() {
    if (hotelRowImageGlobalDebugBound) return;
    hotelRowImageGlobalDebugBound = true;

    // Capture phase lets us observe the interaction before bubbling handlers
    // can be interrupted by stopPropagation in nested UI flows.
    document.addEventListener('click', (event) => {
        const clickedImage = getClosestHotelImageFromEvent(event);
        if (!clickedImage) return;
        const row = clickedImage.closest('.hotel_row_class');
        logHotelRowImageDebug('global click captured', {
            rowId: row ? row.id : null,
            imageSrc: clickedImage.getAttribute('src')
        });
        triggerHotelRowImageController(clickedImage, 'global-click-capture');
    }, true);
}

ensureHotelRowImageGlobalDebugListeners();
...
editClickedHotelDataFunction = function (clickedHotelRowIdName) {
    ...
    confirmNewHotelDataRow = function () {
        ...
        if (document.getElementById('hotel_location_input_id').style.display !== 'none') {
            ...
            const _hotelDbData_update = _findHotelRoomData(hotelNameReadyText);
            let hotelLocationReadyText = _hotelDbData_update ? _hotelDbData_update.hotelLocation : hotelLocationText;
...
function hotelRowImageControllerFunction(event) {
    ...
    let clickedImage = event.target.closest('.hotel_row_image_controller');

    logHotelRowImageDebug('click handler entered', {
        clickedRowId: clickedHotelDataDiv ? clickedHotelDataDiv.id : null,
        imageSrc: clickedImage ? clickedImage.getAttribute('src') : null,
        imageClassName: clickedImage ? clickedImage.className : null
    });
    ...
    if (clickedHotelDataDiv) {
        ...
        let overlayLayer = document.querySelector('.black_overlay');
        if (!overlayLayer) {
            logHotelRowImageDebug('opening action popup and creating overlay', { currentHotelDataDivId });
            overlayLayer = document.createElement('div');
            overlayLayer.classList.add('black_overlay');
            ...
            overlayLayer.addEventListener('click', (overlayEvent) => {
                ...
                logHotelRowImageDebug('overlay clicked -> closing popup', { currentHotelDataDivId });
            ...
            logHotelRowImageDebug('opening action popup using existing overlay', { currentHotelDataDivId });
            overlayLayer.style.opacity = '1';
            overlayLayer.style.pointerEvents = 'auto';
            deleteHotelRowDiv.style.transform = 'translate(-50%, -50%)';
        ...
        runDeleteClickedHotelDataFunction = function () {
            logHotelRowImageDebug('delete action triggered', { currentHotelDataDivId });
        ...
        runEditClickedHotelDataFunction = function () {
            logHotelRowImageDebug('edit action triggered', { currentHotelDataDivId });
        ...
        runDuplicateClickedHotelDataFunction = function () {
            logHotelRowImageDebug('duplicate action triggered', { currentHotelDataDivId });
...
function createHotelDragAndDropMood() {
    ...
    function onPointerDown(event) {
        ...
        logHotelRowImageDebug('pointer down on image', {
            rowId: sourceRow.id,
            pointerId: event.pointerId,
            startX: event.clientX,
            startY: event.clientY,
            imageSrc: handle ? handle.getAttribute('src') : null
        });
    ...
    function onPointerMove(event) {
        ...
        if (!_pointerSession.dragStarted) {
            ...
            logHotelRowImageDebug('drag threshold exceeded -> start drag', {
                rowId: _pointerSession.sourceRow ? _pointerSession.sourceRow.id : null,
                pointerId: event.pointerId
            });
    ...
    function onPointerUp(event) {
        ...
        if (session.dragStarted) {
            logHotelRowImageDebug('pointer up after drag -> finish drag', {
                rowId: session.sourceRow ? session.sourceRow.id : null,
                pointerId: event.pointerId
            });
            finishHotelRowDrag();
            return;
        ...
        event.preventDefault();
        event.stopPropagation();
        logHotelRowImageDebug('pointer up without drag -> treat as click', {
            rowId: session.sourceRow ? session.sourceRow.id : null,
            pointerId: event.pointerId
        });

        triggerHotelRowImageController(session.target, 'pointerup-click-path');
    }

    dropZone.addEventListener('pointerdown', onPointerDown);

    // Fallback: some clicks may bypass pointer flow (platform/browser quirks),
    // so keep a single document-level click handler to guarantee observability.
    if (!hotelRowImageFallbackClickBound) {
        hotelRowImageFallbackClickBound = true;
        document.addEventListener('click', (event) => {
            const clickedImage = event.target.closest('.hotel_row_image_controller');
            if (!clickedImage) return;

            const clickedRow = clickedImage.closest('.hotel_row_class');
            if (!clickedRow) return;

            // If pointer session is active, pointerup handler will decide click vs drag.
            if (_pointerSession) {
                logHotelRowImageDebug('fallback click skipped (pointer session active)', {
                    rowId: clickedRow.id
                });
                return;
            }

            logHotelRowImageDebug('fallback click handler fired', {
                rowId: clickedRow.id,
                imageSrc: clickedImage.getAttribute('src')
            });

            triggerHotelRowImageController(clickedImage, 'dropzone-fallback-click');
        }, true);
    }
}
...
downloadPdfWithCustomName = async function (pdfName) {
    ...
    if (document.getElementById('clint_company_name_input_id').value === 'مغادر') {
        ...
        document.querySelectorAll('.special_package_including_data_background_color_text').forEach(p => {
            p.style.background = 'rgb(13, 63, 46)';;
        });
        // Remove class from elements with specified IDs
        document.getElementById('package_clint_code_number_p_id')?.classList.remove('no_right_padding_for_vid_design_p_class');
        document.getElementById('package_user_code_name_for_later_import_reference_p_id')?.classList.remove('no_right_padding_for_vid_design_p_class');
