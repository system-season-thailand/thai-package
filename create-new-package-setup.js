...
/* async function deletePastMonthRows() {
    try {
        ...
        const rowsToDelete = allRows.filter(row => {
            const rawDateStr = row.package_thai_last_month_date?.trim(); // e.g., "14 November 2025"
        ...
        for (const row of rowsToDelete) {
            const { error } = await supabase
                .from('best_all_package_thai')
                .delete()
                .eq('package_thai_user_current_date', row.package_thai_user_current_date);

            if (error) {
                console.error('❌ Delete failed:', error);
            } else {
                console.log(`✅ Deleted row with date: ${row.package_thai_last_month_date}`);
            }
        }
...
// Run only on certain days of the month
...
// Execute cleanup if within allowed days
if (shouldRunCleanup()) {
    deletePastMonthRows();
} */
...
let randomMessages_1 = ["تعجبني لمن تتحمس", "السيطرة تحت الأمور", "حبة حبة الأمور تمشي", "لا بس بس كذا كثير", "الوضع بدأ يصير خطير", "كيف اقدر اخدمك اليوم", "يعني اجي اكتب بدالك؟"];

// Array of messages for empty password input attempts
let randomMessages_2 = ["حاول مره ثانية", "شكلو مافي فايدة", "انا واثق فيك تقدر", "اليوم مو يومك", "لسى في أمل"];
...
window.addEventListener("load", () => {
    ...
    const checkbox = document.getElementById("mute_website_checkbox"),
        key = "websiteSound";
...
changeInsertHotelDataSystem = function () {
    ...
    if (document.getElementById("hotel_location_input_id").style.display === "none") {
        ...
        document.getElementById("hotel_room_type_description_input_id").onclick = function () {
            createRoomTypeDescripyionDropDown();
            showOverlay("hotel_room_type_description_dropdown");
        };
...
hotelNameInputOptions.forEach((option) => {
    option.addEventListener("click", () => {
        ...
        if (option.textContent !== currentHotelName) {
            ...
            /* Set the autoamtic Bandara Pool Benefits text */
            if (option.textContent === "Bandara Pool Villas Phuket") {
                document.getElementById("hotel_special_room_request_input_id").value = "شامل التزيين على السرير + سلة فواكة ترحيبية + إفطار عائم لمره واحدة";
            }
        }
    });
});
...
hotelBreakFastAmountInputOptions.forEach((option) => {
    option.addEventListener("click", () => {
        ...
        if (option.textContent === "حذف") {
            ...
            lastClickedBreakFastAmountInput.value = `Room Only`;
        } else {
...
lastClickedHotelExtraBedInputOptions.forEach((option) => {
    option.addEventListener("click", () => {
        ...
        } else if (option.textContent === "1 Extra Bed") {
            lastClickedHotelExtraBedInput.value = `+ ${option.textContent} + 1 Extra BF`;
        } else if (option.textContent === "2 Extra Bed") {
            lastClickedHotelExtraBedInput.value = `+ ${option.textContent} + 2 Extra BF`;
        } else if (option.textContent === "3 Extra Bed") {
            lastClickedHotelExtraBedInput.value = `+ ${option.textContent} + 3 Extra BF`;
        } else if (option.textContent === "4 Extra Bed") {
            lastClickedHotelExtraBedInput.value = `+ ${option.textContent} + 4 Extra BF`;
        } else if (option.textContent === "5 Extra Bed") {
            lastClickedHotelExtraBedInput.value = `+ ${option.textContent} + 5 Extra BF`;
        } else if (option.textContent === "6 Extra Bed") {
            lastClickedHotelExtraBedInput.value = `+ ${option.textContent} + 6 Extra BF`;
        } else if (option.textContent === "7 Extra Bed") {
            lastClickedHotelExtraBedInput.value = `+ ${option.textContent} + 7 Extra BF`;
        } else if (option.textContent === "8 Extra Bed") {
            lastClickedHotelExtraBedInput.value = `+ ${option.textContent} + 8 Extra BF`;
        } else if (option.textContent === "9 Extra Bed") {
            lastClickedHotelExtraBedInput.value = `+ ${option.textContent} + 9 Extra BF`;
        } else if (option.textContent === "10 Extra Bed") {
            lastClickedHotelExtraBedInput.value = `+ ${option.textContent} + 10 Extra BF`;
        }
...
// Arabic month names
let innerDatePickerArabicMonths = ["يناير", "فبراير", "مارس", "أبريل", "مايو", "يونيو", "يوليو", "أغسطس", "سبتمبر", "أكتوبر", "نوفمبر", "ديسمبر"];
